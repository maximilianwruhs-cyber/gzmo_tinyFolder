...
import type { ChaosSnapshot } from "../shared/types.ts";
import { Phase, defaultSnapshot } from "../shared/types.ts";
import type { PulseLoop } from "../../plugins/lib/autonomy/pulse.ts";
import type { EmbeddingStore } from "../rag/embeddings.ts";
import { searchVaultHybrid, type SearchResult } from "../rag/search.ts";
import { TaskMemory } from "./memory";
import { safeWriteText } from "./vault_fs";
import { gatherLocalFacts } from "../rag/local_facts.ts";
import { selfEvalAndRewrite } from "../rag/self_eval.ts";
import { gatherVaultStateIndex } from "./vault_state_index";
import { compileEvidencePacket, compileEvidencePacketMulti, renderEvidencePacket, renderEvidencePacketMulti, type EvidencePacket, type EvidencePacketMulti } from "../rag/evidence_packet.ts";
import { formatSearchCitations } from "../rag/citation_formatter.ts";
import { verifySafety } from "../rag/verifier_safety.ts";
import { detectRequiredParts, enforceExactBulletCount, enforceOneSentencePerBullet, enforceRequiredPartsCoverage, shapePreservingFailClosed } from "../rag/response_shape.ts";
import { buildProjectGrounding } from "../rag/project_grounding.ts";
import { checkChainChecklist, enforceChainChecklist } from "./chain_enforce";
import { enforcePerPartCitations } from "../rag/part_citations.ts";
import { appendTaskPerf } from "../shared/perf.ts";
import { OUTPUTS_REGISTRY } from "./outputs_registry";
import { readIntEnv } from "../pipelines/helpers.ts";
import { runDialecticLoop } from "../../plugins/lib/reasoning/dialectic_machine.ts";
import { checkKgCollisions, formatCollisionClarification, kgCollisionEnabled } from "../shared/kg_collision.ts";
import {
  loadTrustState,
  saveTrustState,
  ...
  trustLedgerEnabled,
  updateTrust,
} from "../../plugins/lib/learning/trust_ledger.ts";
import { applyPartQueryHooks, applyPostAnswerHooks, applyPostEvidenceMultiHooks, defaultEngineHooks } from "./engine_hooks";
import { routeJudgeMultipart } from "../clarification/route_judge.ts";
import { atomicWriteJson } from "./vault_fs";
import { SearchPipeline } from "../pipelines/search_pipeline.ts";
import { ThinkPipeline } from "../pipelines/think_pipeline.ts";
import { inferDetailed, OLLAMA_MODEL } from "./inference";
import { inferByRole } from "../../plugins/lib/reasoning/inference_router.ts";
...
import {
  ...
  type ReasoningNode,
  type ReasoningTrace,
} from "../shared/reasoning_trace.ts";
import { readBoolEnv } from "../pipelines/helpers.ts";
import { tickSemanticNoise, semanticNoiseExceeded, defaultSemanticNoiseState } from "../clarification/semantic_noise.ts";
import { runSearchTot } from "../../plugins/lib/reasoning/run_tot_search.ts";
import {
  appendStrategyEntry,
  buildStrategyTips,
  classifyTaskType,
  ...
  learningEnabled,
  loadLedger,
} from "../../plugins/lib/learning/ledger.ts";
...
export async function processTask(
  ...
  try {
    ...
    if (readBoolEnv("GZMO_ENABLE_KNOWLEDGE_GRAPH", false) && vaultRoot) {
      try {
        const { KnowledgeGraph, extractEntities } = await import("../../plugins/lib/knowledge_graph/graph.ts");
