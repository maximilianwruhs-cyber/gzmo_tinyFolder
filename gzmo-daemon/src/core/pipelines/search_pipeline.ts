...
import { searchVaultHybrid, type SearchResult } from "../rag/search";
import { compileEvidencePacket, compileEvidencePacketMulti, renderEvidencePacket, renderEvidencePacketMulti, type EvidencePacket, type EvidencePacketMulti } from "../rag/evidence_packet";
import { detectRequiredParts, enforceExactBulletCount, enforceRequiredPartsCoverage, shapePreservingFailClosed } from "../rag/response_shape";
import { applyPartQueryHooks, applyPostEvidenceMultiHooks } from "../platform/engine_hooks";
import { formatSearchCitations } from "../rag/citation_formatter";
import { verifySafety } from "../rag/verifier_safety";
import { selfEvalAndRewrite } from "../rag/self_eval";
import { buildSystemPrompt, readBoolEnv, readIntEnv, isProofTask, extractExplicitVaultMdPaths } from "./helpers";
import { buildGahClarification, shouldEvidenceGateHalt } from "../clarification/gah_gate";
import { OUTPUTS_REGISTRY } from "../platform/outputs_registry";
  ...
  async prepare(req: TaskRequest): Promise<PipelineContext> {
    ...
    if (embeddingStore) {
      ...
      if (readBoolEnv("GZMO_KG_SEARCH_AUGMENT", false)) {
        try {
          const { KnowledgeGraph } = await import("../../plugins/lib/knowledge_graph/graph");
