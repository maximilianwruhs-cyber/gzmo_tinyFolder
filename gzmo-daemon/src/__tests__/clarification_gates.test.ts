...
import { shouldEvidenceGateHalt, buildGahClarification } from "../core/clarification/gah_gate";
import { dsjNeedsRewrite, dsjRewriteAccepted } from "../core/clarification/dsj_decision";
import { checkThinkClarification } from "../core/clarification/think_clarification";
import { buildTotRetryHint } from "../plugins/lib/reasoning/tot_retry";
...
describe("SearchPipeline.prepare GAH integration", () => {
  test("returns haltReason without retrieval hits when GAH on", async () => {
    ...
    const { SearchPipeline } = await import("../core/pipelines/search_pipeline");
...
describe("ThinkPipeline.prepare clarify integration", () => {
  test("returns haltReason for missing vault file when enabled", async () => {
    ...
    const { ThinkPipeline } = await import("../core/pipelines/think_pipeline");
