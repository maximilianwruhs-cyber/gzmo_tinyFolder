...
import { runWikiLint } from "./src/core/rag/wiki_lint";
import { syncEmbeddings } from "./src/core/rag/embeddings";
...
import { defaultConfig } from "./src/core/shared/types";
import { PulseLoop } from "./src/plugins/lib/autonomy/pulse";
import { VaultWatcher } from "./src/core/platform/watcher";
import { processTask } from "./src/core/platform/engine";
import { TaskMemory } from "./src/core/platform/memory";
import type { TaskEvent } from "./src/core/platform/watcher";
import type { EmbeddingStore } from "./src/core/rag/embeddings";
import type { ChaosSnapshot } from "./src/core/shared/types";
import { TaskDocument } from "./src/core/platform/frontmatter";
...
async function runDiagnostics(flags: ReturnType<typeof parseDoctorFlags>, env: DoctorEnvironment): Promise<DiagnosticsBag> {
  ...
  steps.push(
    await runStep(stepCtx, {
      ...
      run: async () => {
        const { getDropzoneConvertConfig } = await import("./src/core/rag/dropzone_convert");
        const { getDropzoneDedupConfig, DROPZONE_INDEX_REL } = await import("./src/core/rag/dropzone_dedup");
        const { getDropzoneZipConfig } = await import("./src/core/rag/dropzone_zip");
        ...
        const { resolveDropzoneRoot } = await import("./src/core/rag/dropzone_paths");
