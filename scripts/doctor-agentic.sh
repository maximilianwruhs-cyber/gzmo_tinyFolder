...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  ok=0
  actions+=("Run: ./scripts/setup-local.sh --vault /abs/vault --package core --force-env")
elif [[ -x "$REPO_ROOT/scripts/audit-core-env.sh" ]]; then
  if ! "$REPO_ROOT/scripts/audit-core-env.sh" --env-file "$ENV_FILE" 2>&1; then
    warn "audit-core-env reported issues (see above)"
    ok=0
    actions+=("Fix .env per docs/CORE_PROFILE_TUNING.md or re-run setup-local.sh --force-env")
  fi
fi
...
# Ollama / GZMO self-help → $VAULT_PATH/GZMO/SELF_HELP.md (agents read this first when stuck)
if [[ -x "$REPO_ROOT/scripts/local-self-check.sh" ]]; then
  self_args=(--write-vault)
  if (( heal == 1 )); then
    self_args+=(--heal)
  fi
  say "Running: ./scripts/local-self-check.sh ${self_args[*]}"
  if ! "$REPO_ROOT/scripts/local-self-check.sh" "${self_args[@]}"; then
    sc=$?
    if (( sc > 1 )); then
      ok=0
    fi
    actions+=("Read \$VAULT_PATH/GZMO/SELF_HELP.md; re-run: ./scripts/local-self-check.sh --heal --write-vault")
  fi
fi
